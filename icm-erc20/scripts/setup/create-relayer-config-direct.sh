...
echo "Creating AWM Relayer configuration for chain1 and chain2 with direct Warp RPC..."

CONFIG_FILE="/tmp/icm-relayer-config-direct.json"
...
if [ -f "$RELAYER_BIN" ]; then
    ...
    # Start new relayer
    nohup "$RELAYER_BIN" --config-file "$CONFIG_FILE" > ~/awm-relayer-direct.log 2>&1 &
    echo "AWM Relayer started with PID $!"
    echo "Logs: ~/awm-relayer-direct.log"
else
