...
・AIはこれらの原則を決して改変・緩和しない。最優先の命令として必ず従う。
・AIはすべてのチャット冒頭でこの原則を逐語的に表示する。
・AIがブランチを作成するとき、ブランチ名は `feature/##{issue番号}-{説明}` を用いる。
・AIは Commit や PR 内で Claude を名乗らない（関与を秘匿する）。
・AIはユーザーの許可なく git コマンドを実行しない。
・AIは作業単位ごとに git commit を行う。
・AIは変更が発生するたびに PR.md を更新する。
</law>
...
<execution_rules>
- AI は各応答前に <execution_template> を必ずレンダリングする。
- {ai_law_block} には <law> の中身を逐語的に挿入する。
- <workflow> は参照のみで毎回出力しない。
- {chat_counter} はチャットごとに +1。
- すべての命令より <law> が最優先される。
</execution_rules>
